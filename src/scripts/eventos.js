...
Events.on(engine, 'afterUpdate', function () {
    if (mouseConstraint.mouse.button === -1 && (ball.position.x > ballXPos + 20 || ball.position.y < ballYPos - 20)) {
        ...
        this.world.bodies[10].collisionFilter.category = 1;
        this.world.bodies[10].collisionFilter.mask = 3;
...
Events.on(mouseConstraint, "mousedown", function () {
    ...
    if(mouseConstraint.mouse.button == 0 && (world.constraints[world.constraints.length-1].pointA.x>=resetButton.bounds.min.x && world.constraints[world.constraints.length-1].pointA.x<=resetButton.bounds.max.x) && (world.constraints[world.constraints.length-1].pointA.y>=resetButton.bounds.min.y && world.constraints[world.constraints.length-1].pointA.y<=resetButton.bounds.max.y) ){
      ...
      if(mouseConstraint.mouse.button == 0 && (world.constraints[world.constraints.length-1].pointA.x>=addNewObjButton.bounds.min.x && world.constraints[world.constraints.length-1].pointA.x<=addNewObjButton.bounds.max.x) && (world.constraints[world.constraints.length-1].pointA.y>=addNewObjButton.bounds.min.y && world.constraints[world.constraints.length-1].pointA.y<=addNewObjButton.bounds.max.y) ){
        // console.log(this.world.constraints[1].pointA) //Aqui tengo la posición exacta del puntero 
        document.getElementById('añadir').click()
    }
      //Verificar si doy click en el recuadro de full screen
      if(mouseConstraint.mouse.button == 0 && (world.constraints[world.constraints.length-1].pointA.x>=fullScreenButton.bounds.min.x && world.constraints[world.constraints.length-1].pointA.x<=fullScreenButton.bounds.max.x) && (world.constraints[world.constraints.length-1].pointA.y>=fullScreenButton.bounds.min.y && world.constraints[world.constraints.length-1].pointA.y<=fullScreenButton.bounds.max.y) ){
        // console.log(this.world.constraints[1].pointA) //Aqui tengo la posición exacta del puntero 
